...
hostapd_common_add_bss_config() {
	...
	config_add_int dynamic_vlan
	config_add_int dynamic_routing
	config_add_int vlan_naming
	config_add_string vlan_file
	config_add_string vlan_tagged_interface
	config_add_string vlan_br
...
hostapd_set_bss_options() {
	...
	case "$auth_type" in
		...
		eap)
			...
			[ -n "$vlan_naming" ] && append bss_conf "vlan_naming=$vlan_naming" "$N"
			[ -n "$dynamic_vlan" ] && append bss_conf "dynamic_vlan=$dynamic_vlan" "$N"
			[ -n "$dynamic_routing" ] && append bss_conf "dynamic_routing=$dynamic_routing" "$N"
			[ -n "$vlan_file" ] && append bss_conf "vlan_file=$vlan_file" "$N"
			[ -n "$vlan_tagged_interface" ] && append bss_conf "vlan_tagged_interface=$vlan_tagged_interface" "$N"
			[ -n "$vlan_br" ] && append bss_conf "vlan_br=$vlan_br" "$N"
